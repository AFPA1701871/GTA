...
class View_Prestations_Pref_PointManager
{
    ...
    {
        $db = DbConnect::getDb();
        $q = $db->query('SELECT distinct idPrestation, codePrestation, libellePrestation, numeroTypePrestation, libelleTypePrestation, motifRequis, uoRequis, projetRequis,idMotif,codeMotif,libelleMotif, idProjet,codeProjet,libelleProjet, idUo, numeroUo, libelleUo,idTypePrestation, idUtilisateur FROM gta_View_Prestations_Pref_Point WHERE idUtilisateur=' . $idUtilisateur . ' AND idTypePrestation = ' . $idType . ' AND (periode = "' . $mois . '" || isnull(periode))');
        $liste = [];
        if (!$q) {
            return false;
        }
