...
echo '<div class="grid-columns-span-4">';
	echo '<div class="double"></div><input id=searchInList  title="'.texte("infoSearch").'" placeholder="'.texte("mot à chercher").'"/><i class="fa-solid fa-filter fa-margin" title="'.texte("infoSearch").'"></i><div class="vMini"></div>';
	echo '<div class="divfieldset"><fieldset class="fieldset"><legend>'.texte('addholidays').'</legend><label>'.texte('annee').' :</label><input type="number" id="anneeJoursFeries" name="AnneeJoursFeries" value="'.date('Y').'" pattern="'.$regex["num"].'" /><i class="fas fa-floppy-disk fa-margin" id="ajoutJoursFeries"></i></fieldset></div>';
...
echo '<div class="caseListe labelListe  borderbottom" data-name= "DateFermeture">'.texte("DateFermeture").'</div>';
